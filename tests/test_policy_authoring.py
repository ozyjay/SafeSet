...
def test_policy_builder_saves_private_strict_policy(tmp_path):
    ...
    saved = save_policy(
        ROOT / "examples/synthetic_students.csv", destination, example_drafts(), "2"
    )
    ...
    assert validate(
        sanitise(read_csv(ROOT / "examples/synthetic_students.csv"), policy).table, policy
    ).passed
    with pytest.raises(SafetyError, match="overwriting"):
        save_policy(ROOT / "examples/synthetic_students.csv", destination, example_drafts(), "2")


def test_existing_policy_can_be_loaded_and_saved_as_new_file(tmp_path):
    source = ROOT / "examples/synthetic_students.csv"
...
def test_policy_builder_rejects_bad_threshold_before_writing(tmp_path, threshold):
    ...
    with pytest.raises(SafetyError):
        save_policy(
            ROOT / "examples/synthetic_students.csv", destination, example_drafts(), threshold
        )
...
def test_policy_builder_rejects_unclassified_field_and_schema_drift(tmp_path):
    ...
    with pytest.raises(SafetyError):
        save_policy(ROOT / "examples/synthetic_students.csv", destination, drafts, "2")
    drafts = example_drafts()
    drafts.pop("notes")
