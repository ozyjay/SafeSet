...
let restorationAnalysisGuidance = "Please analyse the protected Excel workbook and return a modified .xlsx file that SafeSet can restore. Use the protected workbook as the base. Preserve every original worksheet, row, heading, column order, record_id, entity_id and protected value exactly, including heading spelling and case. Keep each original record_id exactly once on its original worksheet. Append findings as new columns to the right of all protected columns or add separate analysis worksheets. Do not insert result columns among protected columns or change protected values. Do not request the original source, private bundle or passphrase."
